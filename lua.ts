...
import { createRequire } from "https://deno.land/std@0.85.0/node/module.ts";
import { ld } from 'https://deno.land/x/deno_lodash@v0.1.0/mod.ts'

const {isNull, isBoolean, isNumber, isString, isArray, isObject, isEmpty, fromPairs, keys, map, repeat} = ld;

const require = createRequire(import.meta.url);

const { parse: parseLua } = require('./luaparse.js')

const formatLuaString = (string: string, singleQuote: boolean) => (singleQuote ? `'${string.replace(/'/g, "\\'")}'` : `"${string.replace(/"/g, '\\"')}"`)

const formatLuaKey = (string: string, singleQuote: boolean) => (string.match(/^[a-zA-Z_][a-zA-Z_0-9]*$/) ? string : `[${formatLuaString(string, singleQuote)}]`)

const format = (value: any, options = { eol: '\n', singleQuote: true, spaces: 2 }) => {
  options = options || {}
  ...
  const rec = (value: any, i = 0): any => {
    ...
    if (isArray(value)) {
      ...
      if (options.spaces) {
        ...
        return `{${eol}${value.map((e: any) => `${spaces}${rec(e, i + 1)},`).join(eol)}${eol}${spacesEnd}}`
      }
      return `{${value.map((e: any) => `${rec(e, i + 1)},`).join('')}}`
    }
    if (isObject(value)) {
      if (isEmpty(value)) {
        return '{}'
      ...
      if (options.spaces) {
        ...
        return `{${eol}${keys(value)
          .map((key: any) => `${spaces}${formatLuaKey(key, options.singleQuote)} = ${rec(value[key], i + 1)},`)
          .join(eol)}${eol}${spacesEnd}}`
      }
      return `{${keys(value)
        .map((key: any) => `${formatLuaKey(key, options.singleQuote)}=${rec(value[key], i + 1)},`)
        .join('')}}`
    }
...
const luaAstToJson = (ast: any): any => {
  ...
  if (ast.type === 'TableConstructorExpression') {
    ...
    return map(ast.fields, (field: string) => {
...
const parse = (value: string) => luaAstToJson(parseLua(value, { comments: false }))
